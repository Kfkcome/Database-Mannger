...
import com.example.swinedatebaseproject.domain.G2Hau;
import com.example.swinedatebaseproject.service.G2HauService;
...
@RestController
@RequestMapping(value = "g2Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class G2HauController extends CommonController<G2Hau> {


    @Autowired
    public G2HauController(G2HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody G2Hau row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<G2Hau> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody G2Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody G2Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<G2Hau> values) {
