...
@RestController
@RequestMapping(value = "gene",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class GeneController extends CommonController<Gene> {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody Gene object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody Gene object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<Gene> values) {
