...
import com.example.swinedatebaseproject.domain.G1Hau;
import com.example.swinedatebaseproject.service.G1HauService;
...
@RestController
@RequestMapping(value = "g1Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class G1HauController extends CommonController<G1Hau> {


    @Autowired
    public G1HauController(G1HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody G1Hau row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<G1Hau> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody G1Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody G1Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<G1Hau> values) {
