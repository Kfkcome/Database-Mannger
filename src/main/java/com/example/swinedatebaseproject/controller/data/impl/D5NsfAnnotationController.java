...
import com.example.swinedatebaseproject.domain.D5NsfAnnotation;
import com.example.swinedatebaseproject.service.D5NsfAnnotationService;
...
@RestController
@RequestMapping(value = "d5NsfAnnotation",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class D5NsfAnnotationController extends CommonController<D5NsfAnnotation> {


    @Autowired
    public D5NsfAnnotationController(D5NsfAnnotationService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody D5NsfAnnotation row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<D5NsfAnnotation> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody D5NsfAnnotation object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody D5NsfAnnotation object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<D5NsfAnnotation> values) {
