...
import com.example.swinedatebaseproject.domain.Cultivar;
import com.example.swinedatebaseproject.service.CultivarService;
...
@RestController
@RequestMapping(value = "cultivar",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class CultivarController extends CommonController<Cultivar> {


    @Autowired
    public CultivarController(CultivarService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody Cultivar row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<Cultivar> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody Cultivar object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody Cultivar object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<Cultivar> values) {
