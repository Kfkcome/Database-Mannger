...
import com.example.swinedatebaseproject.domain.D1IsuAnnotation;
import com.example.swinedatebaseproject.service.D1IsuAnnotationService;
...
@RestController
@RequestMapping(value = "d1IsuAnnotation",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class D1IsuAnnotationController extends CommonController<D1IsuAnnotation> {


    @Autowired
    public D1IsuAnnotationController(D1IsuAnnotationService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody D1IsuAnnotation row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<D1IsuAnnotation> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody D1IsuAnnotation object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody D1IsuAnnotation object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<D1IsuAnnotation> values) {
