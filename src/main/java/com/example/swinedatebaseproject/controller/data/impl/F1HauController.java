...
import com.example.swinedatebaseproject.domain.F1Hau;
import com.example.swinedatebaseproject.service.F1HauService;
...
@RestController
@RequestMapping(value = "f1Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class F1HauController extends CommonController<F1Hau> {


    @Autowired
    public F1HauController(F1HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody F1Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody F1Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<F1Hau> values) {
