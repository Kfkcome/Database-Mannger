...
import com.example.swinedatebaseproject.domain.A2WhuAnnotation;
import com.example.swinedatebaseproject.service.A2WhuAnnotationService;
...
@RestController
@RequestMapping(value = "a2WhuAnnotation",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class A2WhuAnnotationController extends CommonController<A2WhuAnnotation> {


    @Autowired
    public A2WhuAnnotationController(A2WhuAnnotationService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody A2WhuAnnotation row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<A2WhuAnnotation> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody A2WhuAnnotation object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody A2WhuAnnotation object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<A2WhuAnnotation> values) {
