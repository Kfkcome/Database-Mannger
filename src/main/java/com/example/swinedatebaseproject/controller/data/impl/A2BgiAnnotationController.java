...
import com.example.swinedatebaseproject.domain.A2BgiAnnotation;
import com.example.swinedatebaseproject.service.A2BgiAnnotationService;
...
@RestController
@RequestMapping(value = "a2BgiAnnotation",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class A2BgiAnnotationController extends CommonController<A2BgiAnnotation> {


    @Autowired
    public A2BgiAnnotationController(A2BgiAnnotationService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody A2BgiAnnotation row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<A2BgiAnnotation> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody A2BgiAnnotation object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody A2BgiAnnotation object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<A2BgiAnnotation> values) {
