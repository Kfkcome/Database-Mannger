...
import com.example.swinedatebaseproject.domain.D1Hau;
import com.example.swinedatebaseproject.service.D1HauService;
...
@RestController
@RequestMapping(value = "d1Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class D1HauController extends CommonController<D1Hau> {


    @Autowired
    public D1HauController(D1HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody D1Hau row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<D1Hau> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody D1Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody D1Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<D1Hau> values) {
