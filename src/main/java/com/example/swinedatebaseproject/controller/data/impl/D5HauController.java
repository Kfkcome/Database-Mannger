...
import com.example.swinedatebaseproject.domain.D5Hau;
import com.example.swinedatebaseproject.service.D5HauService;
...
@RestController
@RequestMapping(value = "d5Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class D5HauController extends CommonController<D5Hau> {


    @Autowired
    public D5HauController(D5HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody D5Hau row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<D5Hau> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody D5Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody D5Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<D5Hau> values) {
