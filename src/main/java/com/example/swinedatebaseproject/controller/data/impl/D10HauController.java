...
import com.example.swinedatebaseproject.domain.D10Hau;
import com.example.swinedatebaseproject.service.D10HauService;
...
@RestController
@RequestMapping(value = "d10Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class D10HauController extends CommonController<D10Hau> {


    @Autowired
    public D10HauController(D10HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody D10Hau row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<D10Hau> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody D10Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody D10Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<D10Hau> values) {
