...
import com.example.swinedatebaseproject.domain.D10NsfAnnotation;
import com.example.swinedatebaseproject.service.D10NsfAnnotationService;
...
@RestController
@RequestMapping(value = "d10NsfAnnotation",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class D10NsfAnnotationController extends CommonController<D10NsfAnnotation> {


    @Autowired
    public D10NsfAnnotationController(D10NsfAnnotationService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody D10NsfAnnotation row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<D10NsfAnnotation> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody D10NsfAnnotation object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody D10NsfAnnotation object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<D10NsfAnnotation> values) {
