...
import com.example.swinedatebaseproject.service.Ad1HauService;
...
@RestController
@RequestMapping(value = "ad1Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class Ad1HauController extends CommonController<Ad1Hau> {


    @Autowired
    public Ad1HauController(Ad1HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody Ad1Hau row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<Ad1Hau> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody Ad1Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody Ad1Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<Ad1Hau> values) {
