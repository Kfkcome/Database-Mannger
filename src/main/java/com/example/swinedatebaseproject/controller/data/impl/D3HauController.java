...
import com.example.swinedatebaseproject.domain.D3Hau;
import com.example.swinedatebaseproject.service.D3HauService;
...
@RestController
@RequestMapping(value = "d3Hau",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class D3HauController extends CommonController<D3Hau> {


    @Autowired
    public D3HauController(D3HauService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody D3Hau row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<D3Hau> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody D3Hau object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody D3Hau object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<D3Hau> values) {
