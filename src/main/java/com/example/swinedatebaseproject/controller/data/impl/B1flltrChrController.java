...
import com.example.swinedatebaseproject.domain.B1flltrChr;
import com.example.swinedatebaseproject.service.B1flltrChrService;
...
@RestController
@RequestMapping(value = "b1flltrChr",method = {RequestMethod.DELETE,RequestMethod.GET,RequestMethod.POST})
public class B1flltrChrController extends CommonController<B1flltrChr> {


    @Autowired
    public B1flltrChrController(B1flltrChrService service) {
        super("cds_name");
    ...
    @Override
    public ResponseResult deleteSingleRowById(@RequestBody B1flltrChr row) {
        return deleteSingleRowByIdActual(row);
    }

    @DeleteMapping("/delete-rows")
    @Override
    public ResponseResult deleteRowsByIds(@RequestBody List<B1flltrChr> rows) {
    ...
    @Override
    public ResponseResult updateSingleData(@RequestParam("values")List<String> values, @RequestBody B1flltrChr object) {
        return updateSingleDataActual(values, object);
    }

    @PostMapping("/save-single-data")
    @Override
    public ResponseResult saveSingleData(@RequestBody B1flltrChr object) {
    ...
    @Override
    public ResponseResult deleteBatchOnMainTable(@RequestBody List<B1flltrChr> values) {
